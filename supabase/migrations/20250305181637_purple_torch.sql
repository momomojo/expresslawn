/*
  # Fix Provider Registration Migration

  1. Changes
    - Create provider_profiles table if not exists
    - Update handle_new_user function with provider support
    - Add policies for provider access with existence checks
    - Add verification status index

  2. Security
    - Enable RLS
    - Add proper access policies with checks
...
-- Create policies for provider_profiles with existence checks
DO $$
BEGIN
  -- Select policy
  IF NOT EXISTS (
    ...
    AND policyname = 'Providers can view own profile'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'provider_profiles' 
    AND policyname = 'Providers can insert own profile'
  ) THEN
